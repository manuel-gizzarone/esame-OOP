...
/**
 * la classe contiene il metodo che permette il calcolo della correttezza delle previsioni di nuvolosità su una città in  un dato intervallo di giorni
 * (i cui dati sono sui database delle previsioni e corrente) per mezzo di una soglia di errore inserita come parametro
 * @author emanuele
 *
 */
public class ControllaPrevisioni {
	...
	public JSONObject controllaPrevisioniSoglia (DataMeteo DataInizio,DataMeteo DataFine,String mioNomeCitta,double sogliaErrore) throws DataMeteoException, ParseException, IOException, SogliaErroreNotValidException {
		
		        //per soglia di errore si intende la differenza in valore assoluto tra la media di nuvolosita del giorno tra i dati delle previsioni e quelli correnti
				if(sogliaErrore==0||sogliaErrore>=100)
				throw new SogliaErroreNotValidException();
		
				//secondo quanto richiesto DataInizio e DataFine dovrebbero differire massimo per 5 giorni e con tale ipotesi sono stati sviluppati i metodi seguenti
				...
				//costruisci il Vector <MeteoCitta> dal database corrente degli oggetti la cui data è compresa tra inizio e fine e che corrispondono alla città parametro
				...
				//costruisci il Vector <MeteoCitta> dal database delle previsioni degli oggetti la cui data è compresa tra inizio e fine e che corrispondono alla città parametro
				...
				for(int k=0;k<giorniPredizione;k++) {
					...
					/*
					Vector <MeteoCitta> ArrayDatiPrevisioniConfronto = new Vector <MeteoCitta> ();
					Vector <MeteoCitta> ArrayDatiCorrenteConfronto = new Vector <MeteoCitta> ();
					*/
					...
					for(int i=0;i<ArrayDatiMeteoRisultatoPrevisioni.size();i++) {
						...
						if(DATAMeteoCittaletto.getGiorno()==(dataLavoro.getGiorno()+k)&&(dataLavoro.getGiorno()+k)<=dataLavoro.calcolaLimiteMese(dataLavoro.getMese())&&DATAMeteoCittaletto.getMese()==dataLavoro.getMese()) {
							//ArrayDatiPrevisioniConfronto.add(datoMeteoCittaletto);
							contaPrevisioni++;
